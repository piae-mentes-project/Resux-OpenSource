Shader "Resux/UI/WaveSliderEffect"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float GetOffsetDistanceAddition(float2 pos){
                float offset1 = cos(6.28 * pos.x / _WaveLength - _Time.y * _TimeMulti + 1.67) * _Multi;
                float offset2 = cos(6.28 * pos.x / _WaveLength - _Time.y * _TimeMulti + 3.14) * _Multi;
                float wave1 = 0.8;
                float wave2 = 0.9;
                int v1 = ceil(pos.y - (offset1 + wave1));
            ...
            fixed4 frag(v2f IN) :SV_Target{
                float size = GetOffsetDistanceAddition(IN.texcoord);
                // 直接读取纹理颜色 并且乘上基色
                half4 color = tex2D(_MainTex, IN.texcoord) * IN.color;
                color.a = size * ceil(_Value - IN.texcoord.x) * IN.color.a;
                return color;
            }
            ENDCG
        }

...
namespace Resux.GamePlay
{
    ...
    public static class NoteMove
    {
        ...
        static NoteMove()
        {
            ...
            additiveTimeOffset.Add((int)(GameConfigs.JudgeSetting[JudgeType.Tap][JudgeResult.Bad].y + 0.5f));
            additiveTimeOffset.Add((int)(GameConfigs.JudgeSetting[JudgeType.Hold][JudgeResult.Bad].y + 0.5f));
            additiveTimeOffset.Add((int)(GameConfigs.JudgeSetting[JudgeType.Flick][JudgeResult.Bad].y + 0.5f));
        }
        ...
        private static List<Vector2> CalculatePositionList(NoteMoveInfo noteMoveInfo, int timeOffset, WeightType weightType, JudgeType judgeType)
        {
            ...
            // 配置信息
            var setting = GameConfigs.GamePlaySetting;

            var waterSurface = GameConfigs.GamePlaySetting.WaterSurface;
        ...
        private static float GetAcc(float v, WeightType weight)
        {
            ...
            switch (weight)
            {
                case WeightType.Light:
                    resistance = GameConfigs.GamePlaySetting.LightResistance;
                    break;
                case WeightType.Weight:
                    resistance = GameConfigs.GamePlaySetting.WeightResistance;
                    break;
                case WeightType.Middle:
                default:
                    resistance = GameConfigs.GamePlaySetting.MiddleResistance;
                    break;
            }
